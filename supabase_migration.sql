...
BEGIN
    ...
    IF v_table_name IS NOT NULL THEN
        EXECUTE format('
            ...
            WHERE ... 
            OR cedula = $1 
            LIMIT 1', v_table_name) -- Simplificación robusta
        USING trim(p_cedula)
...
-- Ajustamos la función de validación de médico para que tenga congruencia sintáctica y no colapse al compilar en DB
CREATE OR REPLACE FUNCTION public.validar_medico_tratante(
    p_cedula TEXT
)
RETURNS TABLE (
...
SELECT
    ...
    v.eje_id,
    v.eje_geografico,
    v.nombre_colina AS nombre_asic, -- Ajuste dinámico de columnas si existiera un alias
    v.centro_asic_cod,
    v.nombre_municipio,
    v.municipio_id,
    v.nombre_parroquia,
...
-- fallback o sobreescritura limpia para garantizar congruencia
CREATE OR REPLACE VIEW public.v_nominales_detallado AS
SELECT
    n.id AS nominal_id,
    n.tipo_registro,
