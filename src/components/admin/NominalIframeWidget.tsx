...
export default function NominalIframeWidget({
  ...
  useEffect(() => {
    // Soporte para variables de entorno de Vite (VITE_) y Next.js (NEXT_PUBLIC_)
    const baseUrl =
      ...
      <div className="relative w-full bg-slate-50" style={{ height }}>
        ...
        {loading && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-slate-50/95 p-6 text-center animate-pulse">
            ...
            <div className="mt-6 flex flex-col items-center gap-1 bg-amber-50 rounded-xl p-3 border border-amber-200 border-dashed text-[9.5px] text-amber-805 font-bold uppercase tracking-wide">
              <span>Evitando banner de "Denunciar abuso" clásico usando renderizado directo</span>
              <button 
                onClick={() => setLoading(false)}
                className="mt-2 text-[8.5px] text-[#0B3D5C] hover:underline cursor-pointer"
              >
