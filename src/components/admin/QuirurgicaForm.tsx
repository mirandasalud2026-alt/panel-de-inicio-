...
export default function QuirurgicaForm({ onSuccess, onCancel }: QuirurgicaFormProps) {
  ...
  const [pacienteEncontrado, setPacienteEncontrado] = useState<boolean | null>(null); // null = no buscado, true = existia, false = no existia
  ...
  // Sincronizar búsqueda de paciente
    ...
    try {
      ...
      if (res) {
        setFormData(prev => ({
          ...
          apellido_paciente: res.apellido,
          edad_paciente: res.edad.toString(),
          sexo_paciente: res.sexo,
          telefono_paciente: res.telefono || ''
        }));
  ...
  // Sincronizar búsqueda de médico
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.centro_salud || !formData.cedula_paciente || !formData.nombre_paciente || !formData.cedula_medico || !formData.nombre_medico) {
    ...
    setSuccessMsg('');
    try {
      await nominalService.guardarQuirurgica({
        ...formData,
        edad_paciente: parseInt(formData.edad_paciente) || 0,
        cantidad_intervencion: parseInt(formData.cantidad_intervencion) || 1
      });
      setSuccessMsg('¡Registro Quirúrgico de alta fidelidad y nominales asociados guardados con éxito!');
      setTimeout(() => {
        onSuccess();
      }, 1500);
    } catch (err: any) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50/70 p-4 rounded-2xl border border-slate-100">
        <div>
          <label className="text-[8.5px] font-black tracking-widest text-slate-400 uppercase block mb-1.5 flex items-center gap-1">
            <Calendar size={11} /> Fecha de Intervención *
          </label>
          <input
            type="date"
            name="fecha"
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <div className="flex items-center gap-1.5 border-b border-slate-100 pb-2 mb-1">
          <span className="p-1 px-1.5 rounded-lg bg-[#0B3D5C]/10 text-[#0B3D5C] font-black text-[9px] uppercase">
            PACIENTE
          </span>
          <h4 className="text-[10px] font-black uppercase text-[#0B3D5C] tracking-wide">Ficha del Paciente Quirúrgico</h4>
        </div>
          ...
          <div>
            ...
            {pacienteEncontrado === false && (
              <span className="text-[7.5px] text-amber-600 font-extrabold uppercase mt-1 flex items-center gap-0.5 animate-pulse">
                <Sparkles size={10} /> Nuevo - Se registrará al guardar
              </span>
            )}
          </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-4">
        <div className="flex items-center gap-1.5 border-b border-slate-100 pb-2 mb-1">
          ...
          <h4 className="text-[10px] font-black uppercase text-[#0B3D5C] tracking-wide">Detalle Clínico de la Intervención</h4>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              required
              placeholder="Ej: TRAUMATOLOGIA / CIRUGIA GENERAL"
          ...
          <div>
            ...
            <input
              ...
              required
              placeholder="Ej: HERNIA INGUINAL UNILATERAL"
          ...
          <div>
            <label className="text-[8.5px] font-black tracking-widest text-slate-400 uppercase block mb-1.5">
              Cantidad de Intervención por Especialidad
            </label>
            <input
              type="number"
              name="cantidad_intervencion"
              min="1"
              value={formData.cantidad_intervencion}
              onChange={handleInputChange}
              className="w-full bg-white border border-slate-205 rounded-xl px-3 py-2 text-[10.5px] font-bold"
            />
          </div>
      ...
      {/* BLOQUE DE DATOS DEL MEDICO TRATANTE */}
      <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-100 space-y-4">
        <div className="flex items-center gap-1.5 border-b border-slate-100 pb-2 mb-1">
          <span className="p-1 px-1.5 rounded-lg bg-indigo-50 text-indigo-700 font-black text-[9px] uppercase">
            MÉDICO
          </span>
          <h4 className="text-[10px] font-black uppercase text-[#0B3D5C] tracking-wide">Ficha del Médico Tratante</h4>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-[8.5px] font-black tracking-widest text-slate-400 uppercase block mb-1.5">
              Cédula Médico Tratante *
            </label>
            <div className="relative">
              <input
                type="text"
      ...
      <div className="flex gap-3 pt-4 border-t border-slate-100 justify-end">
        <button
          type="button"
          onClick={onCancel}
          className="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer"
        >
          Cancelar
        </button>
        <button
          ...
          className="flex items-center gap-2 px-6 py-3 bg-[#0B3D5C] hover:bg-[#082E47] text-white rounded-xl text-[10px] font-black uppercase tracking-wider transition-all shadow-md cursor-pointer"
        >
          {submitLoading ? (
          ...
          <span>Guardar Registro Quirúrgico</span>
        </button>
      </div>
    </form>
  );
